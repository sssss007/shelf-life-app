...
## 本轮修复汇总（v1.5.1 → v1.5.5）

### 显示与交互

- **弹窗内容可滚动**（v1.5.1）：录入/编辑弹窗的内容区恒定 `overflow-y:auto` + `min-height:0`，按钮区 `flex:0 0 auto` 不被压缩 —— 修掉「永久删除此物品 / 关闭」在小屏或长表单下显示不全的问题。
- **弹窗高度跟随可视视口**（v1.5.1）：`max-height` 使用 `--app-height`（由 `visualViewport` 实时维护），键盘弹起也不会把按钮顶出屏幕。
- **吸顶搜索栏改为进度插值渐显**（v1.5.3）：滚动 0→40px 时背景 / 细线 / 阴影渐显，扫描过阈值不再突变换高度 —— **零 reflow**，观感更自然。
- **统计卡折叠加迟滞**（v1.5.2）：向下滚过 40px 才折叠、回到 12px 才展开，配合 `overflow-anchor: none` 关掉滚动锚定，消除「点击/滚动时反复抖动」。
- **点按与滚动细节**（v1.5.1）：`touch-action: manipulation`、`@scroll.passive`、`transition: all` 改为具体属性、清理多余的 `will-change`。

### 数据与持久化

- **归档真正落盘**（v1.5.4）：所有「已处理 / 恢复」操作改为按 `id` 回写源数组。此前改的是 `processedProducts` 计算属性生成的**副本**，导致列表/归档/统计不刷新、刷新后已归档商品「复活」。
- **自动持久化**（v1.5.5）：`watch(products, saveData, { deep: true })`，源数据任何改动自动写入 `localStorage`（注册在数据加载之后，避免异常时把空数据回写）。
- **PWA 图标补齐**（v1.5.5）：新增 `icon-192.png` / `icon-512.png`，并写入 `manifest.json` 的 `icons`。此前 HTML 中引用的这两个文件并不存在（404）。
...
```js
const CACHE_VERSION = 'expiry-manager-v1.5.5-release-20260911-1';
```
